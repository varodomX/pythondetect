...
# โค้ดนี้จำเป็นต้องเชื่อมต่ออินเทอร์เน็ตเพื่อดาวน์โหลดภาพจากเว็บไซต์ โดยใช้ไลบรารี requests เพื่อดึงข้อมูลภาพจากเว็บไซต์ และใช้ไลบรารี BeautifulSoup เพื่อวิเคราะห์โครงสร้างของหน้าเว็บและดึง URL ของภาพ
...
def detect_rain(image_path, line_notify_token1, line_notify_token2):
    ...
    if red_area > 15: # ตั้งให้ดูว่าถ้ามากกว่า Pixel ค่อยส่งแจ้งเตือนถ้าน้อยกว่าไม่ต้อง
        ...
        cv2.imwrite('rain_detected_red.jpg', cropped_img)  # บันทึกภาพที่ตรวจพบฝนสีแดงเหลืองเพื่อใช้แสดงผล
    else:
